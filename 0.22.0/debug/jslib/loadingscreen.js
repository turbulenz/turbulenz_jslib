...
LoadingScreen.prototype =
{
    ...
    setTexture: function loadingScreenSetTextureFn(texture)
    {
    ...
    loadAndSetTexture : function loadingScreenLoadAndSetTextureFn(graphicsDevice, requestHandler, mappingTable, name)
    {
        var that = this;
        if (mappingTable)
        {
...
LoadingScreen.create = function loadingScreenCreateFn(gd, md, parameters)
{
    ...
    f.render = function loadingScreenRenderFn(backgroundAlpha, textureAlpha)
    {
        ...
        if (0 < backgroundAlpha)
        {
            ...
            else
            {
                ...
                writer = gd.beginDraw(primitive, 4, posVertexFormats,
                                      posSemantics);
                if (writer)
                {
                    writer(-1, -1);
        ...
        if ((assetTracker) && (backgroundAlpha > 0))
        {
            ...
            writer = gd.beginDraw(primitive, 4, posVertexFormats, posSemantics);
