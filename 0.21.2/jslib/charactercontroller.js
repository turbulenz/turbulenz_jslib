...
CharacterController.prototype =
{
    ...
    setPosition : function setPositionFn(position)
    {
        var md = this.md;
    ...
    setDead : function setDeadFn(dead)
    {
        if (dead && !this.dead)
        {
            this.dead = true;
    ...
    updateExtents : function characterControllerUpdateExtentsFn(position)
    {
    ...
    update : function characterControllerUpdateFn(deltaTime)
    {
        var md = this.md;
        var position;
        if (this.dead)
...
CharacterController.create = function characterControllerCreateFn(gd, id, pd, dynamicsWorld, matrix, params, log)
{
    ...
    c.attach = function attachFn(id)
    {
        ...
        id.addEventListener('paddown', c.onpaddown);
    };
